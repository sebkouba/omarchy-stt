...
2. Restart all daemons: `systemctl --user restart transcribe-daemon recording-daemon`
3. Check for warnings and fix them before completing the task

The user expects a working binary ready to test with all services updated. Both daemons (transcription and recording) must be restarted after any code changes to pick up the new compiled code.
