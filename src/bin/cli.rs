...
use transcribe_rs::{
    clipboard, config::Config, dictation_logger, notifications, paste, performance_log, recording,
    timing,
};
...
enum Commands {
    ...
    Start {
        ...
        /// Capture highlighted text as context for LLM (simulates Ctrl+C before recording)
        #[arg(long)]
        with_clipboard_context: bool,
    },
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cli.command {
        Commands::Start {
            prompt,
            with_clipboard_context,
        } => {
            let config = Config::load()?;
            handle_start(&config, prompt, with_clipboard_context)
        }
        Commands::Stop => {
            let config = Config::load()?;
            handle_stop(&config)
...
fn handle_start(
    ...
    if let Some(prompt_name) = prompt {
        log(
            &format!("Saving prompt name: {}", prompt_name),
            &config.audio.log_file,
        );
    ...
    // Handle clipboard context capture if requested
    const CLIPBOARD_CONTEXT_FILE: &str = "/tmp/ptt_clipboard_context.txt";
    if with_clipboard_context {
        log(
            "Clipboard context capture requested",
            &config.audio.log_file,
        );

        // Save current clipboard content
        let old_clipboard = clipboard::get_clipboard_content().unwrap_or_else(|e| {
            log(
                &format!("WARNING: Failed to read clipboard: {}", e),
                &config.audio.log_file,
            );
            String::new()
        });
        log(
            &format!("Old clipboard: {} bytes", old_clipboard.len()),
            &config.audio.log_file,
        );

        // Simulate Ctrl+C to copy any highlighted text
        if let Err(e) = paste::copy_selection() {
            log(
                &format!("WARNING: Failed to simulate Ctrl+C: {}", e),
                &config.audio.log_file,
            );
            // Continue anyway - user might not have anything selected
        } else {
            // Wait for clipboard to update
            std::thread::sleep(std::time::Duration::from_millis(100));

            // Read new clipboard content
            let new_clipboard = clipboard::get_clipboard_content().unwrap_or_else(|e| {
                log(
                    &format!("WARNING: Failed to read clipboard after Ctrl+C: {}", e),
                    &config.audio.log_file,
                );
                String::new()
            });
            log(
                &format!("New clipboard: {} bytes", new_clipboard.len()),
                &config.audio.log_file,
            );

            // Compare and save if different (text was actually highlighted)
            if !new_clipboard.is_empty() && new_clipboard != old_clipboard {
                log("Clipboard changed - saving context", &config.audio.log_file);
                fs::write(CLIPBOARD_CONTEXT_FILE, &new_clipboard)?;
            } else {
                log(
                    "Clipboard unchanged or empty - no context to save",
                    &config.audio.log_file,
                );
                // Write empty file to signal no context
                fs::write(CLIPBOARD_CONTEXT_FILE, "")?;
            }
        }
    } else {
        // Remove context file if not requested
        let _ = fs::remove_file(CLIPBOARD_CONTEXT_FILE);
        log("Clipboard context not requested", &config.audio.log_file);
    }
    ...
    if let Err(e) = notifications::notify_recording_started() {
        log(
            &format!("WARNING: Notification failed: {}", e),
            &config.audio.log_file,
        );
    }
...
fn handle_stop(config: &Config) -> Result<(), Box<dyn Error>> {
    ...
    if let Err(e) = timing::save_start_time() {
        log(
            &format!("WARNING: Failed to save start timestamp: {}", e),
            &config.audio.log_file,
        );
    }
    ...
    let mut metrics = match timing::PerformanceMetrics::from_start_time() {
        ...
        Err(e) => {
            log(
                &format!("WARNING: Failed to load start timestamp: {}", e),
                &config.audio.log_file,
            );
            None
        }
    ...
    let transcription = match transcribe_file(&audio_file, &config.audio.log_file) {
        Ok(t) => {
            log(
                &format!("Transcription received: {} chars", t.len()),
                &config.audio.log_file,
            );
            t
        }
    ...
    log(
        &format!("Transcription text: '{}'", transcription),
        &config.audio.log_file,
    );
    ...
    let processed_text = if config.transcription_corrections.enabled {
        log(
            "Applying transcription corrections...",
            &config.audio.log_file,
        );
        use std::path::PathBuf;
        ...
        match TranscriptionCorrector::from_file(&corrections_file) {
            Ok(corrector) => {
                ...
                if corrected != transcription {
                    ...
                    log(
                        "No transcription corrections needed",
                        &config.audio.log_file,
                    );
                }
                corrected
            ...
            Err(e) => {
                log(
                    &format!("WARNING: Failed to load transcription corrections: {}", e),
                    &config.audio.log_file,
                );
                transcription.clone()
            }
    ...
    let final_text = if let Ok(prompt_name) = fs::read_to_string(PROMPT_STATE_FILE) {
        ...
        if !prompt_name.is_empty() {
            ...
            log(
                &format!("Prompt requested: {}", prompt_name),
                &config.audio.log_file,
            );

            // Check for clipboard context
            let clipboard_context = if let Ok(context) = fs::read_to_string(CLIPBOARD_CONTEXT_FILE)
            {
                if context.is_empty() {
                    log(
                        "Clipboard context file exists but is empty",
                        &config.audio.log_file,
                    );
                    None
                } else {
                    log(
                        &format!("Clipboard context loaded: {} bytes", context.len()),
                        &config.audio.log_file,
                    );
                    Some(context)
                }
            } else {
                log("No clipboard context file found", &config.audio.log_file);
                None
            };

            // Clean up context file after reading
            let _ = fs::remove_file(CLIPBOARD_CONTEXT_FILE);
            ...
            if is_clear_history_command(&processed_text, &config) {
                ...
                match clear_conversation_history(prompt_name, &config) {
                    ...
                    Err(e) => {
                        log(
                            &format!("ERROR: Failed to clear history: {}", e),
                            &config.audio.log_file,
                        );
                ...
                // Try to process with Groq API, including context if available
                match process_with_groq(
                    &processed_text,
                    prompt_name,
                    ...
                    Ok(result) => {
                        log(
                            &format!("Groq processing successful: '{}'", result.text),
                            &config.audio.log_file,
                        );
                        tool_was_called = result.tool_called;
                        if tool_was_called {
                            log(
                                "Tool was called - will skip paste and show notification only",
                                &config.audio.log_file,
                            );
                        }
                        result.text
                    ...
                    Err(e) => {
                        log(
                            &format!("ERROR: Groq processing failed: {}", e),
                            &config.audio.log_file,
                        );
    ...
    let text = if config.integration.add_space_after_punctuation {
        log(
            "Adding trailing space after punctuation...",
            &config.audio.log_file,
        );
    ...
    if tool_was_called {
        log(
            "Tool called - skipping clipboard and paste, showing notification only",
            &config.audio.log_file,
        );
        ...
        match clipboard::copy_to_clipboard(&text) {
            ...
            Err(e) => {
                log(
                    &format!("ERROR: Clipboard copy failed: {}", e),
                    &config.audio.log_file,
                );
        ...
        if config.integration.auto_paste {
            ...
            match paste::paste_from_clipboard() {
                ...
                Err(e) => {
                    log(
                        &format!("WARNING: Paste failed: {}, clipboard only", e),
                        &config.audio.log_file,
                    );
            ...
            log(
                "Auto-paste disabled in config, clipboard only",
                &config.audio.log_file,
            );
    ...
    if let Some(m) = metrics {
        if let Err(e) = performance_log::log_performance_detailed(&m, &text) {
            log(
                &format!("WARNING: Failed to log performance: {}", e),
                &config.audio.log_file,
            );
        } else {
            log(
            ...
            if llm_processing_triggered && config.dictation_logging.llm_log_enabled {
                ...
                if processed_text.trim() != text.trim() {
                    ...
                    if let Err(e) = dictation_logger::log_llm_correction(
                        ...
                        log(
                            &format!("WARNING: Failed to log LLM correction: {}", e),
                            &config.audio.log_file,
                        );
                    } else {
                    ...
                    log(
                        "LLM processing triggered but no actual changes made (skipped logging)",
                        &config.audio.log_file,
                    );
                }
                ...
                if let Err(e) = dictation_logger::log_basic_dictation(
                    ...
                    log(
                        &format!("WARNING: Failed to log basic dictation: {}", e),
                        &config.audio.log_file,
                    );
                } else {
...
fn process_with_groq(
    ...
    let result = client.complete(&prompt, text, context, prompt_name)?;
...
fn transcribe_file(file: &PathBuf, log_file: &str) -> Result<String, Box<dyn Error>> {
    ...
    log(
        &format!("Looking for transcribe-client at: {:?}", client_path),
        log_file,
    );

    if !client_path.exists() {
        log(
            &format!("ERROR: transcribe-client not found at {:?}", client_path),
            log_file,
        );
        return Err(format!(
    ...
    log(
        &format!("Calling transcribe-client with file: {:?}", file),
        log_file,
    );
    let output = Command::new(&client_path).arg(file).output().map_err(|e| {
        log(
            &format!("ERROR: Failed to execute transcribe-client: {}", e),
            log_file,
        );
        e
    })?;
    ...
    log(
        &format!("transcribe-client exit status: {:?}", output.status),
        log_file,
    );
